...
const useDeploymentSuccess = (
  ...
  useEffect(() => {
    if (isDeploying && invention?.deployingStatus !== 'deployed') {
      setWasDeploying(true);
    }
  }, [isDeploying, invention?.deployingStatus]);
...
export const DeployNftButton: React.FC<DeployNftButtonProps> = ({
  ...
  const openTxApprovalModal = useCallback(
    (gasFeeEstimate: number, type: string, walletAddr?: string) => {
      dispatch(
        Actions.openTxApprovalModal({
          txApprovalModalObj: {
            ...
            open: true,
            type,
            walletAddress: walletAddr || user?.walletAddress
          }
        })
      );
    },
