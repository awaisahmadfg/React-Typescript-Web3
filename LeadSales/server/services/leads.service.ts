...
export const leadHandlers = {
  ...
  update: async (req: Request, res: Response) => {
    ...
    if ("connectionStatus" in updates && updates.connectionStatus != null) {
      ...
      if (existing.connectionStatus === "ACCEPTED" && newStatus !== "ACCEPTED") {
        return res.status(400).json({ message: "Cannot change LinkedIn status: this lead is already connected." });
      }
      // Enforce transition rules: ACCEPTED requires current status to be SENT, except idempotent no-op saves.
      if (newStatus === "ACCEPTED" && existing.connectionStatus !== "SENT" && existing.connectionStatus !== "ACCEPTED") {
      ...
      if (newStatus === "SENT" && existing.connectionStatus === "ACCEPTED") {
        return res.status(400).json({ message: "Cannot send another LinkedIn request: this lead is already connected." });
      }
      if (newStatus === "SENT" && existing.connectionStatus !== "SENT") {
        const [existingConnectionRequest] = await db
          .select({ id: activityTimeline.id })
          .from(activityTimeline)
          .where(
            and(
              eq(activityTimeline.leadId, existing.id),
              eq(activityTimeline.activityType, "linkedin_message"),
              eq(activityTimeline.channel, "conn"),
            ),
          )
          .limit(1);
        if (existingConnectionRequest) {
          return res.status(400).json({ message: "Cannot send another LinkedIn request for this lead." });
        }
      }
    }
  ...
  createActivityTimeline: async (req: Request, res: Response) => {
    ...
    let activityLead: Lead | undefined;
    if (rest.leadId) {
    ...
    const effectiveHappenedAtDate = Number.isNaN(happenedAtDate.getTime())
      ? new Date()
      : happenedAtDate;
    ...
    const getTimelineTime = (entry: { happenedAt: Date; createdAt: Date }) =>
      new Date(entry.happenedAt ?? entry.createdAt).getTime();

    const getLinkedInCycleState = async (leadId: string) => {
      const entries = await db
        .select({
          activityType: activityTimeline.activityType,
          channel: activityTimeline.channel,
          body: activityTimeline.body,
          happenedAt: activityTimeline.happenedAt,
          createdAt: activityTimeline.createdAt,
        })
        .from(activityTimeline)
        .where(eq(activityTimeline.leadId, leadId));

      const latestTime = (predicate: (entry: (typeof entries)[number]) => boolean) =>
        entries.reduce<number | null>((latest, entry) => {
          if (!predicate(entry)) return latest;
          const time = getTimelineTime(entry);
          if (Number.isNaN(time)) return latest;
          return latest == null || time > latest ? time : latest;
        }, null);

      const latestResetAt = latestTime(
        (entry) =>
          entry.activityType === "note" &&
          String(entry.body || "").toLowerCase().includes("linkedin status set to not contacted"),
      );
      const latestRequestAt = latestTime(
        (entry) => entry.activityType === "linkedin_message" && entry.channel === "conn",
      );
      const latestAcceptedAt = latestTime(
        (entry) => entry.activityType === "linkedin_connection_accepted",
      );

      return { latestResetAt, latestRequestAt, latestAcceptedAt };
    };

    if (activityLead && activityType === "linkedin_message" && channel === "conn") {
      if (activityLead.connectionStatus === "ACCEPTED") {
        return res.status(400).json({ message: "Cannot log LinkedIn request: this lead is already connected." });
      }
      if (activityLead.connectionStatus === "SENT") {
        return res.status(400).json({ message: "Cannot log LinkedIn request: this lead already has a pending request." });
      }
    }

    if (activityLead && activityType === "linkedin_connection_accepted") {
      if (activityLead.connectionStatus === "NONE") {
        return res.status(400).json({ message: "Cannot log LinkedIn acceptance before a request is sent." });
      }
      if (activityLead.connectionStatus === "ACCEPTED") {
        return res.status(400).json({ message: "Cannot log LinkedIn acceptance: this lead is already connected." });
      }
      const { latestResetAt, latestRequestAt, latestAcceptedAt } = await getLinkedInCycleState(activityLead.id);
      const activeRequestAt =
        latestRequestAt != null && (latestResetAt == null || latestRequestAt > latestResetAt)
          ? latestRequestAt
          : null;
      if (latestAcceptedAt != null && activeRequestAt != null && latestAcceptedAt > activeRequestAt) {
        return res.status(400).json({ message: "Cannot log another LinkedIn acceptance for this lead." });
      }
    }
    ...
    const shouldMoveToContacted = (stage: string | null | undefined) =>
      String(stage || "").toUpperCase() === "NEW";

    if (activityLead && activityType === "linkedin_message" && channel === "conn" && activityLead.connectionStatus === "NONE") {
      await storage.updateLead(activityLead.id, {
        connectionStatus: "SENT",
        connectionSentAt: effectiveHappenedAtDate,
        ...(shouldMoveToContacted(activityLead.stage) ? { stage: "CONTACTED" } : {}),
      });
    }
    if (activityLead && activityType === "linkedin_connection_accepted" && activityLead.connectionStatus === "SENT") {
      await storage.updateLead(activityLead.id, {
        connectionStatus: "ACCEPTED",
        connectionAcceptedAt: effectiveHappenedAtDate,
        ...(shouldMoveToContacted(activityLead.stage) ? { stage: "CONTACTED" } : {}),
      });
    }
