...
const coerceFiniteNumber = (v: unknown): number | undefined => {
...
const clampDelay = (v: unknown) => {
  const numeric = coerceFiniteNumber(v);
  return numeric === undefined ? undefined : Math.max(0, Math.floor(numeric));
};

const clampTaskCount = (v: unknown) => {
  const numeric = coerceFiniteNumber(v);
  return numeric === undefined ? undefined : Math.min(4, Math.max(0, Math.floor(numeric)));
};
...
function sanitizeDelays(body: PlanUpdateBody): PlanUpdateBody {
  ...
  const emailCount = clampTaskCount(body.emailTaskCount);
  const linkedinCount = clampTaskCount(body.linkedinTaskCount);
  const callCount = clampTaskCount(body.callTaskCount);
  ...
  if (emailCount !== undefined) result.emailTaskCount = emailCount;
  if (linkedinCount !== undefined) result.linkedinTaskCount = linkedinCount;
  if (callCount !== undefined) result.callTaskCount = callCount;
  if (body.taskScheduleStartDate !== undefined) {
    result.taskScheduleStartDate = body.taskScheduleStartDate;
  }
  return result;
}
...
const getEnabledPlanChannels = (plan: Plan): PlanChannel[] => {
  ...
  if (raw.includes("call") || Number(plan.callTaskCount ?? 0) > 0) {
    channels.add("CALL");
  }
...
const getTaskCountByChannel = (plan: Plan, channel: PlanChannel): number => {
  if (channel === "EMAIL") return Math.max(0, Number(plan.emailTaskCount ?? MAX_TASKS_PER_CHANNEL));
  if (channel === "LINKEDIN") return Math.max(0, Number(plan.linkedinTaskCount ?? MAX_TASKS_PER_CHANNEL));
  return Math.max(0, Number(plan.callTaskCount ?? MAX_TASKS_PER_CHANNEL));
};
...
const syncPlanTasks = async (plan: Plan) => {
  ...
  const baseDate = plan.taskScheduleStartDate ? new Date(plan.taskScheduleStartDate) : new Date();

  for (const channel of channels) {
    const delayDays = getDelayByChannel(plan, channel);
    const intervalDays = delayDays + 1;
    const taskType = getTaskTypeByChannel(channel);
    const taskCount = getTaskCountByChannel(plan, channel);

    for (let i = 0; i < taskCount; i += 1) {
      const dueDate = toDateString(addDays(baseDate, intervalDays * i));
...
export const planHandlers = {
  ...
  remove: async (req: Request, res: Response) => {
    ...
    // TEAM_LEAD can delete only when they are the HEAD of at least one team linked to this plan.
    if (currentUser.role === "TEAM_LEAD") {
      const planTeamIds = await storage.getPlanTeamIds(plan.id);
      const allTeams = await storage.getTeams();
      const headedTeamIds = new Set(
        allTeams.filter((team) => team.leadId === currentUser.id).map((team) => team.id),
      );
      const isHeadOfLinkedTeam = planTeamIds.some((teamId) => headedTeamIds.has(teamId));
      if (!isHeadOfLinkedTeam) {
        return res.status(403).json({
          message: "Only the Team Lead heading a plan-linked team can delete this plan.",
        });
      }
    }
