...
const getManagedTeamIds = async (userId: string): Promise<Set<string>> => {
...
const canTeamLeadManageTask = async (userId: string, task: { leadId?: string | null; planId?: string | null }) => {
  const managedTeamIds = await getManagedTeamIds(userId);
  if (managedTeamIds.size === 0) return false;

  if (task.leadId) {
    const [lead] = await storage.getLeads({ id: task.leadId, includeStale: "true" });
    return !!lead?.teamId && managedTeamIds.has(lead.teamId);
  }

  if (task.planId) {
    const planTeamIds = await storage.getPlanTeamIds(task.planId);
    return planTeamIds.some((teamId) => managedTeamIds.has(teamId));
  }
...
export const taskHandlers = {
  list: async (req: Request, res: Response) => {
    ...
    const enrichTasksWithLeadMeta = async (scopedTasks: typeof tasks) => {
      ...
      const planAssigneeNames = new Map<string, string[]>();
      const planAssigneeIds = new Map<string, string[]>();
      await Promise.all(
        uniquePlanIds.map(async (planId) => {
          ...
          const assignments = await storage.getPlanAssignments(planId);
          const uniqueAssignedUserIds = Array.from(new Set(assignments.map((a) => a.userId).filter(Boolean)));
          planAssigneeIds.set(planId, uniqueAssignedUserIds);
          const assignedUsers = await Promise.all(uniqueAssignedUserIds.map((userId) => storage.getUser(userId)));
          const names = assignedUsers
            .map((u) => u?.name?.trim())
            .filter((name): name is string => !!name);
          planAssigneeNames.set(planId, names);
        }),
      );
      return scopedTasks.map((task) => {
        ...
        return {
          ...
          assigneeNames: task.planId ? (planAssigneeNames.get(task.planId) ?? []) : [],
          assigneeIds: task.planId ? (planAssigneeIds.get(task.planId) ?? []) : [],
        };
      });
    };
    ...
    // Also include plans where this user is explicitly assigned via Manage AEs.
    // This ensures assignees can see shared plan tasks even when team-link visibility is missing.
    const allPlans = await storage.getPlans();
    await Promise.all(
      allPlans.map(async (plan) => {
        const assignments = await storage.getPlanAssignments(plan.id);
        if (assignments.some((assignment) => assignment.userId === currentUser.id)) {
    ...
    const shouldMergePlanSharedTasks =
      (currentUser.role === "AE" || currentUser.role === "SDR") &&
      !!filters.userId &&
      filters.userId === currentUser.id;

    if (!shouldMergePlanSharedTasks) {
      return res.json(await enrichTasksWithLeadMeta(visibleTasks));
    }

    // AE/SDR frontend intentionally sends ?userId=self. Merge plan-shared tasks back in.
    const allTasks = await storage.getTasks({});
    const planSharedTasks = allTasks.filter((task) => !!task.planId && visiblePlanIds.has(task.planId));
    const mergedById = new Map<string, (typeof allTasks)[number]>();
    visibleTasks.forEach((task) => mergedById.set(task.id, task));
    planSharedTasks.forEach((task) => mergedById.set(task.id, task));
    return res.json(await enrichTasksWithLeadMeta(Array.from(mergedById.values())));
  },
  ...
  update: async (req: Request, res: Response) => {
    ...
    let isPlanAssignee = false;
    if (previous.planId) {
      const planAssignments = await storage.getPlanAssignments(previous.planId);
      isPlanAssignee = planAssignments.some((assignment) => assignment.userId === currentUser.id);
    }
    const isManagingTeamLead =
      currentUser.role === "TEAM_LEAD" && (await canTeamLeadManageTask(currentUser.id, previous));
    const canEditAsCreator = isCreator && currentUser.role !== "TEAM_LEAD";
  ...
  remove: async (req: Request, res: Response) => {
    ...
    const isManagingTeamLead =
      currentUser.role === "TEAM_LEAD" && (await canTeamLeadManageTask(currentUser.id, existing));
    const canDelete =
      currentUser.role === "ADMIN" ||
      isManagingTeamLead;
    if (!canDelete) {
