...
export default function LeadDetailsPage() {
  ...
  // NONE = request can be sent, SENT = request can be accepted,
  // ACCEPTED = lifecycle is complete and cannot be restarted.
  const canLogConnectionRequest = lead.connectionStatus === ConnectionStatus.NONE;
  ...
  const linkedinCurrentLabel =
    lead.connectionStatus === ConnectionStatus.ACCEPTED
      ...
      : lead.connectionStatus === ConnectionStatus.SENT
        ? "Request Sent"
        : "Not Connected";
  const linkedinNextActionLabel =
    lead.connectionStatus === ConnectionStatus.ACCEPTED
      ? "No further connection action"
      : canLogConnectionAccepted
    ...
    if (!canLogConnectionRequest) {
      if (lead.connectionStatus === ConnectionStatus.SENT) {
        toast({
          ...
          description: `LinkedIn connection request for ${lead.firstName} ${lead.lastName} is already pending. Mark it accepted before sending another request.`,
          variant: "destructive",
        });
        return false;
      }
      toast({
        ...
        description: "This lead is already connected. A new LinkedIn request cycle is not allowed.",
        variant: "destructive",
      });
      return false;
    }
    try {
      const logged = await logActivity("linkedin_message", "LinkedIn connection request sent", undefined, { channel: "conn" });
      if (!logged) throw new Error("Failed to update LinkedIn status");
      ...
      toast({ title: "Error", description: error?.message || "Failed to mark LinkedIn connection as sent.", variant: "destructive" });
      return false;
    }
  };
  ...
  const handleConnectionAccepted = async () => {
    ...
    try {
      const logged = await logActivity("linkedin_connection_accepted", "LinkedIn connection request accepted");
      if (!logged) throw new Error("Failed to update LinkedIn status");
      ...
      toast({ title: "Error", description: error?.message || "Failed to mark LinkedIn connection as accepted.", variant: "destructive" });
      return false;
    }
  };
                ...
                <Button className="w-full" onClick={async () => {
                  if (linkedinMessageType === "conn" && !canLogConnectionRequest) {
                    toast({
                      ...
                      description:
                        lead.connectionStatus === ConnectionStatus.ACCEPTED
                          ? "This lead is already connected. A new LinkedIn request cycle is not allowed."
                          : "This lead already has a pending connection request.",
                      variant: "destructive",
                    });
                    return;
                  }
                  if (linkedinMessageType === "accepted" && !canLogConnectionAccepted) {
                    toast({
                      ...
                      description:
                        lead.connectionStatus === ConnectionStatus.ACCEPTED
                          ? "This lead is already connected."
                          : "First log a connection request, then mark it as accepted.",
                      variant: "destructive",
                    });
                    return;
                  }
                  ...
                  if (linkedinMessageType === "accepted") {
                    const saved = await handleConnectionAccepted();
                    if (saved) setIsLinkedInOpen(false);
                    return;
                  }
                  if (linkedinMessageType === "conn") {
                    const saved = await handleConnectionSent();
                    if (saved) setIsLinkedInOpen(false);
                    return;
                  }
                  await logActivity(
